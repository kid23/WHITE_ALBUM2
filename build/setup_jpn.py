...
def zip_dar(): 
    f = zipfile.ZipFile('../output/darjpn.zip','w',zipfile.ZIP_DEFLATED) 
    for dirpath, dirnames, filenames in os.walk('../output/darjpn/'): 
        for filename in filenames: 
            f.write(os.path.join(dirpath,filename), filename) 
    f.close() 

zip_dar()    
data = open("../output/darjpn.zip", "rb").read()
excludes = list()
